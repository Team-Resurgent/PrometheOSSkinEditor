...
namespace PrometheOSSkinEditor
{
    public class ImGuiController : IDisposable
    {
        ...
        {
            var arialFontData = ResourceLoader.GetEmbeddedResourceBytes("PrometheOSSkinEditor.Resources.arial.ttf", typeof(ImGuiController).GetTypeInfo().Assembly);
            var arialPinnedFontData = GCHandle.Alloc(arialFontData, GCHandleType.Pinned);

            var arialBoldFontData = ResourceLoader.GetEmbeddedResourceBytes("PrometheOSSkinEditor.Resources.arialbold.ttf", typeof(ImGuiController).GetTypeInfo().Assembly);
            var arialBoldPinnedFontData = GCHandle.Alloc(arialBoldFontData, GCHandleType.Pinned);

            var controllerFontData = ResourceLoader.GetEmbeddedResourceBytes("PrometheOSSkinEditor.Resources.controller.otf", typeof(ImGuiController).GetTypeInfo().Assembly);
            var controllerPinnedFontData = GCHandle.Alloc(controllerFontData, GCHandleType.Pinned);

            var crossoverFontData = ResourceLoader.GetEmbeddedResourceBytes("PrometheOSSkinEditor.Resources.crossover.ttf", typeof(ImGuiController).GetTypeInfo().Assembly);
            var crossoverPinnedFontData = GCHandle.Alloc(crossoverFontData, GCHandleType.Pinned);
            ...
            ImGui.GetIO().Fonts.AddFontFromMemoryTTF(arialPinnedFontData.AddrOfPinnedObject(), arialFontData.Length, 18, null, glyphRangeHandle.AddrOfPinnedObject());

            ImGui.GetIO().Fonts.AddFontFromMemoryTTF(arialBoldPinnedFontData.AddrOfPinnedObject(), arialBoldFontData.Length, 18, null, glyphRangeHandle.AddrOfPinnedObject());
            ImGui.GetIO().Fonts.AddFontFromMemoryTTF(arialBoldPinnedFontData.AddrOfPinnedObject(), arialBoldFontData.Length, 25, fontConfig, glyphRangeHandle.AddrOfPinnedObject());
            ImGui.GetIO().Fonts.AddFontFromMemoryTTF(arialBoldPinnedFontData.AddrOfPinnedObject(), arialBoldFontData.Length, 32, fontConfig, glyphRangeHandle.AddrOfPinnedObject());

            ImGui.GetIO().Fonts.AddFontFromMemoryTTF(controllerPinnedFontData.AddrOfPinnedObject(), controllerFontData.Length, 18, fontConfig, glyphRangeHandle.AddrOfPinnedObject());

            ImGui.GetIO().Fonts.AddFontFromMemoryTTF(crossoverPinnedFontData.AddrOfPinnedObject(), crossoverFontData.Length, 32, fontConfig, glyphRangeHandle.AddrOfPinnedObject());



            ImGui.GetIO().Fonts.Build();

            arialPinnedFontData.Free();
            arialBoldPinnedFontData.Free();
            controllerPinnedFontData.Free();
            crossoverPinnedFontData.Free();
            glyphRangeHandle.Free();
            ImGuiNative.ImFontConfig_destroy(fontConfig);
        }

        public unsafe int CreateTextureFromResource(string resourceName)
        {
        ...
        public unsafe int CreateTextureFromFile(string path, out int width, out int height)
        {
            ...
            GL.BindTexture(TextureTarget.Texture2D, texture);
            GL.TexStorage2D(TextureTarget2d.Texture2D, mips, SizedInternalFormat.Rgba8, resourceImage.Width, resourceImage.Height);
            LabelObject(ObjectLabelIdentifier.Texture, texture, Path.GetFileName(path));
        ...
        {
            _splashTexture = CreateTextureFromResource("PrometheOSSkinEditor.Resources.splash.jpg");
            _installerTexture = CreateTextureFromResource("PrometheOSSkinEditor.Resources.installer.png");
        }
        ...
        public void CreateDeviceResources()
        {
            ...
            string VertexSource = ResourceLoader.GetEmbeddedResourceString("PrometheOSSkinEditor.Resources.imgui-vertex.glsl");
            string FragmentSource = ResourceLoader.GetEmbeddedResourceString("PrometheOSSkinEditor.Resources.imgui-frag.glsl");
        ...
        public void DisposeOwnedTexture(int textureId)
        {
            foreach (var texture in _ownedTextures)
            {
